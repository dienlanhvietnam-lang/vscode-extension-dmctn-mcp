...
function testServerManifest() {
  ...
  assert.equal(m.sha256, "2437B418C53FACBE7C09B54C9027CC3822555F6633CCB68C871C4FF59EDFF101");
...
function testBundledServerPaths() {
  ...
  assert.equal(needsServerBootstrap(root, "0.18.0", undefined), true);
  assert.equal(needsServerBootstrap(root, "0.18.0", "0.18.0"), !isBundledServerReady(root));
...
async function testAcquireZipLocal() {
  const srcZip = path.resolve(
    ...
    "release",
    "local-coding-tools-mcp-v0.18.0-customer.zip"
  );
