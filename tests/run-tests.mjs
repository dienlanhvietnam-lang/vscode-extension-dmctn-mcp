...
function testServerManifest() {
  ...
  assert.equal(m.version, "0.10.0");
  assert.ok(m.downloadUrl.includes("github.com"));
  assert.match(m.sha256, /^[A-F0-9]{64}$/);
  assert.equal(m.sha256, "8F1A2B87DC6615ECB5DF3964D80261C0F11B562EF3282AD97F2A0085F5E29E68");
...
function testBundledServerPaths() {
  ...
  assert.equal(needsServerBootstrap(root, "0.10.0", undefined), true);
  assert.equal(needsServerBootstrap(root, "0.10.0", "0.10.0"), !isBundledServerReady(root));
...
function testSyncWorkspace() {
  ...
  assert.ok(agent.includes("local-coding-tools/*"));
  fs.rmSync(tmp, { recursive: true, force: true });
...
async function testAcquireZipLocal() {
  const srcZip = path.resolve(
    ...
    "release",
    "local-coding-tools-mcp-v0.10.0-customer.zip"
  );
